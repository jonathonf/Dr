var options = {};
options.preferences = {};
options.preferences.safeMode = 'server';
options.preferences.customAttributes = '';

var asciidocify = {
  options: function(option_string) {
      options.preferences.customAttributes = option_string;
  },
  load: function() {
...
function buildAsciidoctorOptions() {
  // Preferences
  var preferences = {};
  if (self && self.options && self.options.preferences) {
    preferences = self.options.preferences;
  } else {
    preferences.safeMode = 'server';
  }
  // Default attributes
  var attributes = 'showtitle icons=font@ platform=opal platform-opal env=browser env-browser chart-engine=chartist data-uri! source-highlighter=prettify';
  var href = window.location.href;
  var fileName = href.split('/').pop();
  var fileExtension = fileName.split('.').pop();
  if (preferences.customAttributes) {
    attributes = attributes.concat(' ').concat(preferences.customAttributes);
  }
  if (fileExtension !== '') {
...
function convertSanitizeAndRender() {
  ...
  if (document.characterSet.toUpperCase() != 'UTF-8') {
    try {
      ...
      showHTML(convertToHTML(decodeURIComponent(escape(document.firstChild.innerText))));
    } catch (decodeError) {
    ...
    showHTML(convertToHTML(document.firstChild.innerText));
  }
}
...
function appendStyles() {
  ...
  asciidoctorLink.href = 'css/asciidoctor.css';
  ...
  var prettifyLink = document.createElement('link');
  prettifyLink.rel = 'stylesheet';
  prettifyLink.id = 'prettify-style';
  prettifyLink.href = 'css/prettify.css';
  document.head.appendChild(prettifyLink);
}
